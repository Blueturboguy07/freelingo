...
| the built pack loads in `packages/core`'s loader                                         | **DID NOT RUN** — the step exists (`real-pack.test.ts`, `FREELINGO_REAL_PACK`, fails rather than skips when the variable is set and the pack is absent) and lives in `validate-es`, which skipped. Locally the file **skips** with the variable unset, which is the designed behaviour: 1 test file skipped, 6 tests                                                                | pack-ci `validate-es` |

...
| `pnpm test`                                                                              | **GREEN on the handed-over sha** — 1,316 passed / 6 skipped on `42b0be3`, first attempt, and on `859f3fb` before it. It FAILED on both attempts of `86f2430` with the same 1,316 passing and `Timeout calling "onTaskUpdate"`: an intermittent reporter RPC timeout. Three failures seen across the round's shas, all of them the slowest runs (227 s and 212 s) against the successes' 153 s — a runner-speed effect, not a suite change. The denominator is not quotable: every push of this file adds an attempt (B20) | ci.yml                               |
...
evidence: it is a flake, and the failures are the slow runs. It reappeared once more
after this section was written — on `682f7f2`, the commit carrying the report's own
corrections, at 212.36 s, cleared by a single `--failed` re-run — which turned the speed
correlation from a hypothesis into an out-of-sample prediction that held. The tally itself
is not worth quoting, for the reason B20 now gives: every push of this report runs `ci.yml`
again and moves the denominator, so any "N of M" is stale before it lands.
`docs/P2-BLOCKERS.md` §B20 carries the full table of attempts.

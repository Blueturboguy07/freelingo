/**
 * The platform gates' falsifier corpus.
 *
 * INV-PLAT-01 and INV-PLAT-02 are P0 ids that shipped with tests and no corpus, and their
 * falsifying inputs are REPO STATES rather than function arguments: a hand-typed
 * dependency version, a committed file under `apps/mobile/ios`. Writing them down is
 * still worth doing — a reader who has never seen the failure cannot otherwise tell what
 * `platform-gates.test.ts` is defending against, and the CI job that really proves
 * INV-PLAT-02 (`prebuild-determinism`) lives in another file entirely.
 *
 * The fixtures say so in `mustNotBe` rather than pretending to be executable.
 */
...
const EXPECTED_IDS: readonly string[] = ['INV-PLAT-01', 'INV-PLAT-02'];
...
describe('the platform falsifier corpus', () => {
